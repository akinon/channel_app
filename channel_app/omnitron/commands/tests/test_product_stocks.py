...
from channel_app.omnitron.commands.product_stocks import ProcessStockBatchRequests
from channel_app.omnitron.constants import BatchRequestStatus


class TestProcessStockBatchRequests(BaseTestCaseMixin):
    """
    Test case for ProcessStockBatchRequests
    
    run: python -m unittest channel_app.omnitron.commands.tests.test_product_stocks.TestProcessStockBatchRequests
    """

    def setUp(self) -> None:
        self.instance = ProcessStockBatchRequests(
    ...
    @patch.object(BaseClient, 'get_instance')
    @patch.object(ProcessStockBatchRequests, 'get_products')
