...
from channel_app.omnitron.commands.product_images import ProcessImageBatchRequests
from channel_app.omnitron.constants import BatchRequestStatus


class TestProcessImageBatchRequests(BaseTestCaseMixin):
    """
    Test case for ProcessImageBatchRequests
    
    run: python -m unittest channel_app.omnitron.commands.tests.test_product_stocks.ProcessImageBatchRequests
    """

    def setUp(self) -> None:
        self.instance = ProcessImageBatchRequests(
    ...
    @patch.object(BaseClient, 'get_instance')
    @patch.object(ProcessImageBatchRequests, 'get_products')
